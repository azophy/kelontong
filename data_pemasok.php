...
$form_value=fillForm('pemasok', $elemen, $_GET[id]);
?>
		...
		<div id="wrapper">
			...
			<div id="content" class="row">
				...
				<div id="form_area" class="column">
					<h3>Data Pemasok <?php echo ($tampil_baru)?'Baru':'Lama'; ?></h3>
					<div class="box" style="width: 320px;">
						<form id="form_pemasok" method="POST" action="<?php echo $_SERVER['PHP_SELF']; ?>">
							<input type="hidden" name="id" value="<?php echo $form_value[id]; ?>" />
							<div class="row">
								<label>Kode Pemasok</label> <input type="text" name="kode" value="<?php echo $form_value[kode]; ?>" />
							</div>
							<div class="row">
								<label>Nama Pemasok</label> <input type="text" name="nama" value="<?php echo $form_value[nama]; ?>" />
							</div>
							<div class="row">
				...
				<div id="table_area" class="column" style="width: 650px;">
					...
					<div style="border: 1px solid #BFBFBF; overflow-y:none; overflow-x: scroll;">
						<table id="tabel_pemasok" class="sortable">
							<?php
							$query=	"SELECT pemasok.id AS id, 
										pemasok.nama AS nama, 
										pemasok.kode AS kode, 
										pemasok.alamat AS alamat,
										pemasok.telepon AS telepon,
										pemasok.fax AS fax,
										pemasok.kontak AS kontak
									FROM pemasok
									LIMIT $posisi, $n_data_halaman"; //echo $query;
							$elemen=array('Kode pemasok' => 'kode', 'Nama pemasok' => 'nama', 'Telepon' => 'telepon', 'Fax' => 'fax', 'Contact Person' => 'kontak', 'Alamat' => 'alamat');
			...
			function confirm_delete(val) {
				return confirm('Apakah anda benar-benar ingin menghapus pemasok: '+val+'?\nPerhatian: cek terlebih dahulu transaksi-transaksi yang berhubungan dengan pemasok ini.')
			}
			</script>
			<style type="text/css">
				#form_pemasok label { width: 110px; }
